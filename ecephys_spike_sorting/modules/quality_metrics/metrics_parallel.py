...
def calculate_metrics(cluster_id, 
                      spike_times, 
                      spike_clusters, 
                      amplitudes, 
                      channel_map, 
    ...
    ------
    cluster_id : int
        cluster_id to use for computation
    ...
    for epoch in epochs:
        ...
        isi_viol = calculate_isi_violations(cluster_id, spike_times[in_epoch], spike_clusters[in_epoch], total_units, params['isi_threshold'], params['min_isi'])
        
        presence_ratio = calculate_presence_ratio(cluster_id, spike_times[in_epoch], spike_clusters[in_epoch], total_units)

        firing_rate = calculate_firing_rate(cluster_id, spike_times[in_epoch], spike_clusters[in_epoch], total_units)
        
        amplitude_cutoff = calculate_amplitude_cutoff(cluster_id, spike_clusters[in_epoch], amplitudes[in_epoch], total_units)
        
        isolation_distance, l_ratio, d_prime, nn_hit_rate, nn_miss_rate = calculate_pc_metrics(cluster_id,
        ...
        silhouette_score = calculate_silhouette_score(cluster_id,
                                                       spike_clusters[in_epoch], 
                                                       total_units,
                                                       pc_features[in_epoch,:,:],
                                                       pc_feature_ind,
        ...
        max_drift, cumulative_drift = calculate_drift_metrics(cluster_id,
                                                       spike_times[in_epoch],
                                                       spike_clusters[in_epoch], 
                                                       total_units,
                                                       spike_depths,
        ...
        metrics = pd.concat((metrics, pd.DataFrame(data= OrderedDict((('cluster_id', [cluster_id]),
...
def calculate_isi_violations(cluster_id, spike_times, spike_clusters, total_units, isi_threshold, min_isi):

    for_this_cluster = (spike_clusters == cluster_id)

    viol_rate, num_violations = isi_violations(spike_times[for_this_cluster], 
...
def calculate_presence_ratio(cluster_id, spike_times, spike_clusters, total_units):

    for_this_cluster = (spike_clusters == cluster_id)
    ratio = presence_ratio(spike_times[for_this_cluster], 
                                                   min_time = np.min(spike_times), 
                                                   max_time = np.max(spike_times))
...
def calculate_firing_rate(cluster_id, spike_times, spike_clusters, total_units):

    min_time = np.min(spike_times)
    max_time = np.max(spike_times)

    for_this_cluster = (spike_clusters == cluster_id)
    fr = firing_rate(spike_times[for_this_cluster], 
                                    min_time = np.min(spike_times),
                                    max_time = np.max(spike_times))
...
def calculate_amplitude_cutoff(cluster_id, spike_clusters, amplitudes, total_units):

    for_this_cluster = (spike_clusters == cluster_id)
    ac = amplitude_cutoff(amplitudes[for_this_cluster])

    return [ac]


def calculate_pc_metrics(cluster_id,
                         spike_clusters, 
                         total_units,
                         channel_map, 
                         pc_features, 
    ...
    for idx, cluster in enumerate(cluster_ids):
        ...
        for_unit = np.squeeze(spike_clusters == cluster)
        
        pc_max = np.argmax(np.mean(pc_features[for_unit, 0, :],0))
        
        peak_channels[cluster] = feature_inds[pc_max]
        actual_channels[cluster] = channel_map[int(peak_channels[idx])]

    idx = np.argmin(np.abs(cluster_ids - cluster_id))
    ...
    if all_pcs.shape[0] > 10:
        ...
        iso_dist, l_ratio = mahalanobis_metrics(all_pcs, all_labels, cluster_id)

        d_prime = lda_metrics(all_pcs, all_labels, cluster_id)

        nn_hit_rate, nn_miss_rate = nearest_neighbors_metrics(all_pcs, all_labels, cluster_id, max_spikes_for_nn, n_neighbors)
    ...
    return [iso_dist], [l_ratio], [d_prime], [nn_hit_rate], [nn_miss_rate] 


def calculate_silhouette_score(cluster_id,
                                spike_clusters, 
                                 total_units,
                                 pc_features, 
                                 pc_feature_ind,
    ...
    SS = np.empty((total_units,))
    SS[:] = np.nan

    for idx, j in enumerate(cluster_ids):
        
        inds = np.in1d(cluster_labels, np.array([cluster_id,j]))
        X = all_pcs[inds,:]
        labels = cluster_labels[inds]
        
        try:
            SS[j] = silhouette_score(X, labels)
        except ValueError:
            SS[j] = np.nan
...
def calculate_drift_metrics(cluster_id,
                            spike_times,
                            spike_clusters,
                            total_units,
                            spike_depths,
    ...
    depths_for_cluster = spike_depths[in_cluster]
